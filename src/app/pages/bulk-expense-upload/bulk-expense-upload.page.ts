...
import { BulkExpenseUploadService, ExpenseUploadRecord, ExpenseValidationResult, ExpenseUploadResult } from '../../services/bulk-expense-upload.service';
import { Wallet } from '../../interfaces/interfaces';
import { WalletService } from '../../services/wallet.service';

@Component({
  selector: 'app-bulk-expense-upload',
  templateUrl: './bulk-expense-upload.page.html',
  styleUrls: ['./bulk-expense-upload.page.scss'],
})
export class BulkExpenseUploadPage implements OnInit {
  currentStep = 1;
  selectedFile: File | null = null;
  selectedWalletId: number | null = null;
  wallets: Wallet[] = [];
  validationResult: ExpenseValidationResult | null = null;
  uploadResult: ExpenseUploadResult | null = null;
  parsedRecords: ExpenseUploadRecord[] = [];
  isLoading = false;
  isDragOver = false;
  ...
  constructor(
    ...
    private bulkExpenseUploadService: BulkExpenseUploadService,
  ...
  async validateFile() {
    ...
    try {
      ...
      this.parsedRecords = await this.bulkExpenseUploadService.parseExcelFile(this.selectedFile);
      
      // Validar usando el servicio
      const response = await this.bulkExpenseUploadService.validateFile(this.selectedFile, this.selectedWalletId).toPromise();
      console.log('Bulk expense upload validation response:', response);
  ...
  async processBulkUpload() {
    ...
    const alert = await this.alertController.create({
      ...
      message: `¿Estás seguro de que quieres procesar ${this.validationResult.validRecords} gastos?`,
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel'
  ...
  private async executeBulkUpload() {
    this.isLoading = true;
    const loading = await this.loadingController.create({
      message: 'Procesando cargue masivo de gastos...',
    });
    await loading.present();
    ...
    try {
      ...
      const response = await this.bulkExpenseUploadService.processBulkUpload(requestData).toPromise();
      console.log('Bulk expense upload process response:', response);
